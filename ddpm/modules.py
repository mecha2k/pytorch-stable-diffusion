...
class Unet(nn.Module):
    def __init__(
        ...
        init_dim = default(
            init_dim, dim
        )  # Initial dimension of the feature map extracted from the input image
        self.init_conv = nn.Conv2d(
            input_channels, init_dim, 7, padding=3
        )  # Convert the input image into a feature map

        dims = [
            init_dim,
            *map(lambda m: dim * m, dim_mults),
        ]  # Builds multiples of the initial dimensions, used for the encoder and the decoder
        in_out = list(
            zip(dims[:-1], dims[1:])
        )  # Builds a map of input-output dimensions for the encoder and the decoder

        block_klass = partial(
            ResnetBlock, groups=resnet_block_groups
        )  # Initializes the number of groups for Group Normalization
